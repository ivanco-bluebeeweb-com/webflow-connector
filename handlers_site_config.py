...
async def list_components(ctx, params: ListComponentsParams) -> ActionResult:
    ...
    return ActionResult.ok(WebflowComponentList(items=items))
...
async def get_component_content(ctx, params: GetComponentContentParams) -> ActionResult:
    ...
    return ActionResult.ok(WebflowComponentContent(component_id=params.component_id, nodes=body.get("nodes", []) if isinstance(body, dict) else []))
...
async def list_redirects(ctx, params: ListRedirectsParams) -> ActionResult:
    ...
    return ActionResult.ok(WebflowRedirectList(items=items))
...
async def create_redirect(ctx, params: CreateRedirectParams) -> ActionResult:
    ...
    return ActionResult.ok(WebflowRedirect(id=r.get("id", ""), from_url=r.get("fromUrl", params.from_url), to_url=r.get("toUrl", params.to_url)), message="Redirect created.")
...
async def delete_redirect(ctx, params: DeleteRedirectParams) -> ActionResult:
    ...
    return ActionResult.ok(DeleteResult(id=params.redirect_id), message="Redirect deleted.")
